...
<div class="max-w-7xl mx-auto px-4 flex gap-6">
    {{-- Bagian kiri (video utama + komentar) --}}
    <div class="flex-1">
            ...
            <div class="p-6">
                ...
                <div class="flex justify-between items-center">
                    <p class="text-gray-500 text-sm">
                        Uploaded by {{ $video->user->username }} • {{ $video->views }} views
                    </p>
        ...
        <div class="bg-[#080808] rounded-lg shadow-md p-6">
            ...
            <form method="POST" action="{{ route('comments.store', $video) }}" class="mb-4">
                ...
                <textarea name="content" rows="3" placeholder="Add a comment..." required
                    class="bg-[#080808] text-white w-full border border-neutral-800 rounded-lg px-4 py-2 focus:ring-2 focus:ring-red-500"></textarea>
                @error('content')
            ...
            @forelse ($video->comments as $comment)
                <div class="border-b border-neutral-700 py-3">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="font-semibold text-white">{{ $comment->user->username }} : {{ $comment->content }}</p>
        ...
        <a href="{{ route('videos.show', $related->id) }}" class="flex gap-2 hover:bg-[#1a1a1a] p-2 rounded-lg transition">
            <div class="w-40 h-24 bg-gray-700 rounded-md overflow-hidden">
                @if ($related->thumbnail_path)
                    <img src="{{ asset('storage/' . $related->thumbnail_path) }}" 
                        alt="{{ $related->title }}" 
